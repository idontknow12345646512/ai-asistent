...
const HF_MODELS = [
  { id:'black-forest-labs/FLUX.1-schnell',         name:'FLUX.1 Schnell ⚡', desc:'Nejrychlejší' },
  { id:'stabilityai/stable-diffusion-xl-base-1.0', name:'SDXL 🎨',          desc:'Kvalitní' },
  { id:'runwayml/stable-diffusion-v1-5',            name:'SD 1.5 🖼️',        desc:'Klasika' },
  { id:'Lykon/dreamshaper-8',                       name:'DreamShaper 8 ✨',  desc:'Stylizovaný' },
  { id:'prompthero/openjourney-v4',                 name:'OpenJourney v4 🚀', desc:'Umělecký' },
]

const AI_MODELS = [
  { id:'default',    name:'Gemini Auto',   desc:'Nejlepší dostupný model' },
  { id:'gemma-3-12b',name:'Gemma 3 12B',   desc:'Otevřený model od Google' },
  { id:'gemma-3-27b',name:'Gemma 3 27B',   desc:'Výkonnější otevřený model' },
]
...
function renderMD(text, isDark) {
  if(!text) return ''
  const cb=isDark?'#0d1117':'#f6f8fa', cbrd=isDark?'#30363d':'#d0d7de'
  const inBg=isDark?'rgba(110,118,129,0.2)':'rgba(175,184,193,0.2)'
  return text
    .replace(/```(\w*)\n?([\s\S]*?)```/g,(_,lang,code)=>{
      const lbl=lang?`<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 12px;background:${isDark?'#161b22':'#eaeef2'};border-bottom:1px solid ${cbrd};font-size:11px;color:${isDark?'#8b949e':'#57606a'};font-family:monospace;border-radius:8px 8px 0 0"><span style="font-weight:600">${lang}</span><span>code</span></div>`:''
    ...
    .replace(/`([^`]+)`/g,`<code style="background:${inBg};padding:2px 6px;border-radius:4px;font-size:12px;font-family:monospace;border:1px solid ${cbrd}">$1</code>`)
...
// ─── Themes ───────────────────────────────────────────────────────────────────
const TH = {
  dark:     {bg:'#0f1117',side:'#13161f',hdr:'rgba(15,17,23,.96)',txt:'#e8eaf0',muted:'#5a6178',border:'#1e2230',accent:'#6c8fff',purple:'#a855f7',active:'#1c2035',aiB:'#1a1d2a',inBg:'#13161f',iaBg:'#0f1117',inBrd:'#2a2f42',btn:'#1a1d2a',modal:'#13161f',pill:'#1a1d2a',ua:'#3d4460',scrl:'#2a2f42',card:'#1e2230',tag:'#1c2035',success:'#22543d',succ:'#68d391',danger:'#e53e3e',isDark:true},
  light:    {bg:'#f4f6fb',side:'#fff',hdr:'rgba(244,246,251,.96)',txt:'#1a1d2a',muted:'#7a849a',border:'#e2e6f0',accent:'#4c6ef5',purple:'#9333ea',active:'#eef1ff',aiB:'#fff',inBg:'#fff',iaBg:'#f4f6fb',inBrd:'#d8dde8',btn:'#edf0f7',modal:'#fff',pill:'#edf0f7',ua:'#8898b0',scrl:'#c8cdd8',card:'#edf0f7',tag:'#eef1ff',success:'#f0fff4',succ:'#276749',danger:'#dc2626',isDark:false},
  midnight: {bg:'#070a12',side:'#0c0f1c',hdr:'rgba(7,10,18,.97)',txt:'#dde4f5',muted:'#4a526a',border:'#151b2e',accent:'#818cf8',purple:'#c084fc',active:'#141728',aiB:'#111525',inBg:'#0c0f1c',iaBg:'#070a12',inBrd:'#1e2540',btn:'#111525',modal:'#0c0f1c',pill:'#111525',ua:'#2d3555',scrl:'#1e2540',card:'#141728',tag:'#141728',success:'#1a3a2a',succ:'#6ee7b7',danger:'#e53e3e',isDark:true},
}
...
function TypingText({ text, isDark, markdown }) {
  const [displayed, setDisplayed] = useState('')
  const [done, setDone] = useState(false)
  useEffect(() => {
    setDisplayed(''); setDone(false)
    let i = 0
    // Rychlost závisí na délce — kratší texty jsou rychlejší
    const speed = text.length > 500 ? 4 : text.length > 200 ? 6 : 10
  ...
  if (done && markdown) return <div style={{fontSize:14,lineHeight:1.7,wordBreak:'break-word'}} dangerouslySetInnerHTML={{__html:renderMD(text,isDark)}}/>
  return <div style={{fontSize:14,lineHeight:1.65,whiteSpace:'pre-wrap',wordBreak:'break-word'}}>{displayed}<span style={{opacity:done?0:1,transition:'opacity .2s',animation:done?'none':'blink 1s infinite'}}>▊</span></div>
}
...
function GenImg({ imageData, mimeType, prompt, modelId, t }) {
  ...
  const dl=()=>{const a=document.createElement('a');a.href=src;a.download=`ai-${Date.now()}.png`;a.click()}
  return (
    <div>
      <div style={{fontSize:12,color:t.muted,marginBottom:8,display:'flex',alignItems:'center',gap:6}}>
        {Ic.magic} HuggingFace · <strong style={{color:t.purple}}>{modelName}</strong>
      </div>
...
// ─── Quiz with progress + score ───────────────────────────────────────────────
function QuizCard({ questions, t }) {
  const [cur,setCur]=useState(0)
  const [answers,setAnswers]=useState({})
  const [done,setDone]=useState(false)
  if(!questions?.length) return <div style={{padding:'12px 14px',background:t.aiB,borderRadius:'16px 16px 16px 4px',border:`1px solid ${t.border}`,fontSize:13,color:t.muted}}>Kvíz nemá žádné otázky.</div>
  ...
  if(done){
    ...
    return (
      <div style={{padding:'18px',background:t.aiB,borderRadius:'16px 16px 16px 4px',border:`1px solid ${t.border}`,textAlign:'center'}}>
        ...
        <div style={{fontSize:12,color:t.muted,marginTop:10,marginBottom:16}}>
          {pct>=80?'Výborně! Skvělý výkon!':pct>=60?'Dobrá práce!':pct>=40?'Slušný výsledek, zkuste to znovu.':'Nebojte, příště to půjde lépe!'}
        </div>
        <button onClick={()=>{setAnswers({});setCur(0);setDone(false)}} style={{padding:'10px 24px',borderRadius:9,background:t.accent,color:'#fff',fontSize:13,fontWeight:600,border:'none',cursor:'pointer',fontFamily:'inherit'}}>
          🔄 Zkusit znovu
        </button>
      </div>
    )
    ...
    <div style={{padding:'14px 16px',background:t.aiB,borderRadius:'16px 16px 16px 4px',border:`1px solid ${t.border}`}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:7}}>
        {q.options?.map((opt,i)=>{
          ...
          if(sel!==undefined){if(isC){bg=t.success;clr=t.succ;brd=t.succ}else if(isSel){bg='#2a1a1a';clr='#fca5a5';brd='#6a2d2d'}}
          return (
            <button key={i} onClick={()=>sel===undefined&&setAnswers(p=>({...p,[cur]:i}))} disabled={sel!==undefined}
              ...
              <span>{opt}</span>
              {sel!==undefined&&isC&&<span style={{marginLeft:'auto',fontWeight:600}}>✓</span>}
            </button>
          )
        })}
      </div>
      ...
      {answers[cur]!==undefined&&(
        <div style={{marginTop:12}}>
          {cur<total-1?(
            <button onClick={()=>setCur(c=>c+1)} style={{width:'100%',padding:'9px',borderRadius:8,background:t.accent,color:'#fff',fontSize:13,fontWeight:600,border:'none',cursor:'pointer',fontFamily:'inherit'}}>
              Další otázka →
            </button>
          ):(
            <button onClick={()=>setDone(true)} style={{width:'100%',padding:'9px',borderRadius:8,background:'#f59e0b',color:'#fff',fontSize:13,fontWeight:600,border:'none',cursor:'pointer',fontFamily:'inherit'}}>
              Zobrazit výsledky 🏆
            </button>
          )}
        </div>
...
function VoiceBtn({ t, onTranscript }) {
  const [on,setOn]=useState(false),[txt,setTxt]=useState('')
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:8}}>
      <button onClick={toggle} style={{display:'flex',alignItems:'center',gap:6,padding:'5px 11px',borderRadius:20,background:on?'#ef4444':t.btn,color:on?'#fff':t.muted,border:`1px solid ${on?'#ef4444':t.border}`,fontSize:12,fontWeight:on?600:400,fontFamily:'inherit',cursor:'pointer',transition:'all .2s'}}>
        {on?<><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg> Stop</>:<>{Ic.mic} Hlas</>}
      </button>
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:4,marginTop:5,flexWrap:'wrap'}}>
        ...
        <button onClick={()=>rat===-1?setShowFix(true):feedback(-1)} style={{display:'flex',padding:'3px 6px',borderRadius:5,background:rat===-1?'#2a1a1a':t.btn,color:rat===-1?'#fca5a5':t.muted,border:`1px solid ${rat===-1?'#6a2d2d':t.border}`,cursor:'pointer'}}>{Ic.thumbDn}</button>
      </div>
      {showFix&&(
...
function SettingsModal({ t, themeName, setThemeName, sysPmt, setSysPmt, onClose, isLoggedIn, userId, memory, setMemory, aiModel, setAiModel }) {
  const [tmp,setTmp]=useState(sysPmt),[memList,setMemList]=useState([]),[tab,setTab]=useState('appearance')
  useEffect(()=>{
    if(tab==='memory'&&isLoggedIn) supabase.from('user_memory').select('*').eq('user_id',userId).order('created_at',{ascending:false}).limit(30).then(({data})=>setMemList(data||[]))
  ...
  const personas=[
    ...
    {label:'Lektor / učitel',val:'Jsi trpělivý lektor. Vysvětluješ jednoduše s příklady a přizpůsobuješ se tempu studenta.'},
    {label:'Sokrates (filosofický)',val:'Jsi sokratovský filosof. Odpovídáš otázkami, vedeš dialog, pomáháš hledat pravdu.'},
  ]
  ...
  return (
    <>
      ...
      <div onClick={e=>e.stopPropagation()} style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',zIndex:50,width:'min(540px,96vw)',maxHeight:'88vh',display:'flex',flexDirection:'column',background:t.modal,border:`1px solid ${t.border}`,borderRadius:18,fontFamily:"'DM Sans',sans-serif",overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 20px 20px'}}>
          ...
          {tab==='model'&&(
            <>
              <div style={{fontSize:11,fontWeight:600,color:t.muted,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:10}}>AI Model pro chat</div>
              <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
                {AI_MODELS.map(m=>(
                  <button key={m.id} onClick={()=>setAiModel(m.id)} style={{padding:'12px 14px',borderRadius:10,border:`1.5px solid ${aiModel===m.id?t.accent:t.border}`,background:aiModel===m.id?t.accent+'18':t.btn,color:aiModel===m.id?t.accent:t.txt,fontSize:13,textAlign:'left',cursor:'pointer',fontFamily:'inherit',transition:'all .15s'}}>
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <strong>{m.name}</strong>
                      {aiModel===m.id&&<span style={{color:t.accent}}>{Ic.check}</span>}
                    </div>
                    <div style={{fontSize:12,color:aiModel===m.id?t.accent:t.muted,marginTop:3,fontWeight:400}}>{m.desc}</div>
                  </button>
                ))}
              </div>
              <div style={{padding:'12px 14px',borderRadius:10,background:t.tag,border:`1px solid ${t.border}`,fontSize:12,color:t.muted,lineHeight:1.6}}>
                💡 <strong style={{color:t.txt}}>Gemma modely</strong> jsou open-source od Google a běží přes Gemini API. Gemma 3 27B je největší a nejschopnější otevřený model.
              </div>
            </>
          )}
          {tab==='behavior'&&(
          ...
          {tab==='memory'&&(
            <>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                <div style={{fontSize:11,fontWeight:600,color:t.muted,textTransform:'uppercase',letterSpacing:'.08em'}}>Uložená paměť {memList.length>0&&`(${memList.length})`}</div>
                {memList.length>0&&<button onClick={async()=>{if(isLoggedIn){await supabase.from('user_memory').delete().eq('user_id',userId);setMemList([])}}} style={{fontSize:11,color:t.danger,cursor:'pointer',background:'none',border:'none',fontFamily:'inherit'}}>Smazat vše</button>}
              </div>
              ...
              {memList.map(m=>(
                <div key={m.id} style={{display:'flex',alignItems:'flex-start',gap:8,padding:'9px 12px',borderRadius:9,background:t.tag,border:`1px solid ${t.border}`,marginBottom:7,animation:'fadeIn .2s ease'}}>
          ...
          {tab==='about'&&(
            <div style={{fontSize:13,color:t.muted,lineHeight:1.7}}>
              ...
              <p style={{textAlign:'center',fontWeight:600,color:t.txt,fontSize:15,marginBottom:2}}>AI Asistent</p>
              <p style={{textAlign:'center',fontSize:11,marginBottom:4}}><span style={{background:'#f59e0b22',color:'#f59e0b',padding:'2px 8px',borderRadius:4,fontWeight:600}}>BETA</span></p>
              <p style={{textAlign:'center',marginBottom:16,fontSize:12}}>Powered by Google Gemini + HuggingFace</p>
              {[['🤖 Chat','Gemini 3.1 Flash'],['💭 Thinking','Gemini 2.5 Flash/Pro'],['🔶 Gemma','Gemma 3 12B / 27B'],['🔴 Live','Gemini 2.0 Flash Live'],['🎨 AI Obrázky','HuggingFace FLUX/SD'],['📷 Fotografie','Unsplash API'],['🎙️ Hlas','Web Speech API'],['🧠 Paměť','Supabase PostgreSQL'],['🔒 Auth','Supabase Auth']].map(([k,v])=>(
        ...
        {(tab==='appearance'||tab==='behavior'||tab==='model')&&(
          <div style={{padding:'0 20px 20px',display:'flex',gap:8,justifyContent:'flex-end',flexShrink:0}}>
            ...
            <button onClick={()=>{setSysPmt(tmp);onClose()}} style={{padding:'8px 16px',borderRadius:8,background:t.accent,color:'#fff',fontSize:13,fontWeight:600,cursor:'pointer',border:'none',fontFamily:'inherit'}}>Uložit</button>
          </div>
        )}
      </div>
    </>
...
export default function Chat({ session }) {
  ...
  // Animace - id nových zpráv
  const [newIds,setNewIds]       = useState(new Set())
  // Typing animation ids
  ...
  // Emotion detection with debounce
  useEffect(()=>{
  ...
  async function saveMsg(cid,role,content,type='text',meta=null){
    const{data}=await supabase.from('messages').insert({conversation_id:cid,role,content,type,image_url:meta?JSON.stringify(meta):null}).select().single()
    return data
  }
  ...
  const sendQuiz=async()=>{
    ...
    try{
      ...
      else{await saveMsg(cid,'user',tmp.content,'text',null);await saveMsg(cid,'assistant',JSON.stringify(qs),'text',null);setMsgs(p=>[...p.filter(m=>!m._tmp),{...tmp,_tmp:false},aMsg])}
      setNewIds(s=>{const n=new Set(s);n.add(aMsg.id);setTimeout(()=>setNewIds(s2=>{const n2=new Set(s2);n2.delete(aMsg.id);return n2}),600);return n})
  ...
  const send=useCallback(async()=>{
    ...
    if(isLocal){
      setConvs(p=>p.map(c=>{if(c.id!==cid)return c;const title=isFirst?userText.slice(0,38)+(userText.length>38?'…':''):c.title;return{...c,title,messages:[...(c.messages??[]),tmpUser]}}))
    }else{
      setMsgs(p=>[...p,tmpUser])
    ...
    try{
      ...
      if(result.type==='image_search'){
        ...
        // Spustit typing animaci
        setTypingIds(s=>{const n=new Set(s);n.add(nid);return n})
        setTimeout(()=>setTypingIds(s=>{const n=new Set(s);n.delete(nid);return n}), Math.min(result.text?.length*12||3000,8000))
      }
      // Animace slide-in
      ...
      if(isLocal){
        ...
        else if(aMsg.type==='generated_image')await saveMsg(cid,'assistant',aMsg.content,'image',{imageData:aMsg._imageData,mimeType:aMsg._mimeType,prompt:aMsg._prompt,modelId:hfModel})
  ...
  },[input,atts,loading,activeConv,msgs,isLoggedIn,imgMode,sysPmt,thinking,memory,token,hfModel,aiModel]) // eslint-disable-line
  ...
  function getImgData(msg){
    ...
    if(msg.image_url){try{const p=JSON.parse(msg.image_url);return{...msg,_images:Array.isArray(p)?p:undefined,_imageData:p.imageData,_mimeType:p.mimeType,_prompt:p.prompt,_modelId:p.modelId,_query:msg.content}}catch{return msg}}
    return msg
  }

  const modeColor=imgMode==='generate_image'?t.purple:t.accent
  const ltxt={chat:null,image_search:'🔍 Hledám fotografie…',generate_image:'🎨 Generuji obrázek (30–60 sekund)…'}
  const phs={chat:thinking?'💭 Hluboké přemýšlení (Gemini 2.5)…':'Napište zprávu… (Enter = odeslat)',image_search:'🔍 Popište co hledáte…',generate_image:'🎨 Popište obrázek co nejdetailněji…'}

  const currentModelName = AI_MODELS.find(m=>m.id===aiModel)?.name || 'Gemini'
    ...
    <div style={{display:'flex',height:'100dvh',overflow:'hidden',background:t.bg,color:t.txt,fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .msg-old{animation:fadeIn .25s ease both}
      ...
      {sideOpen&&(
        <aside className="sidebar" style={{width:272,background:t.side,borderRight:`1px solid ${t.border}`,display:'flex',flexDirection:'column',flexShrink:0}}>
          <div style={{padding:'13px 12px',borderBottom:`1px solid ${t.border}`,display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <div>
                ...
                <span style={{marginLeft:6,fontSize:10,background:'#f59e0b22',color:'#f59e0b',padding:'1px 5px',borderRadius:3,fontWeight:600}}>BETA</span>
              </div>
            </div>
            <div style={{display:'flex',gap:4}}>
              <button className="ib" onClick={()=>{setSearchOpen(o=>!o);setTimeout(()=>searchRef.current?.focus(),100)}} style={{color:t.muted,display:'flex',padding:6,borderRadius:6,background:searchOpen?t.active:'transparent'}} title="Hledat chaty">{Ic.search}</button>
          ...
          <div style={{padding:'10px 11px',borderTop:`1px solid ${t.border}`}}>
            {isLoggedIn?(
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <div style={{width:32,height:32,borderRadius:9,background:t.accent+'33',border:`1px solid ${t.accent}55`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:700,color:t.accent,flexShrink:0}}>{userInitial}</div>
                <div style={{flex:1,minWidth:0}}>
      ...
      <main style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
        ...
        <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 14px',height:52,background:t.hdr,borderBottom:`1px solid ${t.border}`,backdropFilter:'blur(12px)',flexShrink:0,gap:8}}>
          <div style={{display:'flex',alignItems:'center',gap:8,minWidth:0}}>
            ...
            {aiModel!=='default'&&<span style={{fontSize:10,background:t.purple+'22',color:t.purple,padding:'2px 7px',borderRadius:4,flexShrink:0,fontWeight:600}}>{currentModelName}</span>}
          </div>
          <div style={{display:'flex',gap:4,alignItems:'center',flexShrink:0}}>
            <button className="ib" onClick={()=>setShowStarred(s=>!s)} style={{display:'flex',padding:'5px 8px',borderRadius:7,background:showStarred?'#f59e0b22':t.btn,color:showStarred?'#f59e0b':t.muted,border:`1px solid ${showStarred?'#f59e0b':t.border}`}} title="Oblíbené">{showStarred?Ic.starF:Ic.star}</button>
            <button className="ib" onClick={exportChat} style={{display:'flex',padding:'5px 8px',borderRadius:7,background:t.btn,color:t.muted}} title="Export chatu">{Ic.export}</button>
            <button className="ib" onClick={()=>setMdMode(m=>!m)} style={{display:'flex',alignItems:'center',gap:2,padding:'5px 8px',borderRadius:7,background:mdMode?t.accent+'22':t.btn,color:mdMode?t.accent:t.muted,border:`1px solid ${mdMode?t.accent:t.border}`,fontSize:11}} title="Markdown">MD</button>
            {isLoggedIn&&(
              <button className="ib" onClick={()=>setThinking(x=>!x)} style={{display:'flex',alignItems:'center',gap:3,padding:'5px 8px',borderRadius:7,background:thinking?t.purple+'33':t.btn,color:thinking?t.purple:t.muted,border:`1px solid ${thinking?t.purple:t.border}`,fontSize:11}} title="Hluboké přemýšlení (Gemini 2.5)">
                {Ic.brain}{thinking&&<span style={{animation:'thinkPulse 1.5s infinite'}}> ON</span>}
              </button>
            )}
            <button className="ib" onClick={()=>setShowSet(true)} style={{display:'flex',padding:'5px 8px',borderRadius:7,background:t.btn,color:t.muted}} title="Nastavení">{Ic.gear}</button>
            <button className="ib" onClick={()=>setThemeName(n=>n==='dark'?'light':n==='light'?'midnight':'dark')} style={{display:'flex',padding:'5px 8px',borderRadius:7,background:t.btn,color:t.muted}} title="Změnit téma">
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 14px',display:'flex',flexDirection:'column',gap:12}}>
          {displayMsgs.length===0&&!loading&&(
            <div style={{textAlign:'center',marginTop:'7vh',padding:'0 16px',animation:'fadeIn .5s ease'}}>
              ...
              {!showStarred&&<>
                <div style={{fontSize:13,color:t.muted,marginBottom:18}}>{isLoggedIn?'Chat · HF Obrázky · Fotografie · Kvízy · Live · Hlasový vstup · Gemma':'Začněte psát — přihlášení není potřeba'}</div>
              ...
              <div key={msg.id} className={isNew?'msg-new':'msg-old'} style={{display:'flex',gap:8,justifyContent:msg.role==='user'?'flex-end':'flex-start',alignItems:'flex-start'}}>
                ...
                <div style={{maxWidth:isWide?'94%':'80%',minWidth:40}}>
                  ...
                  {msg.type==='image_search'?(
                    ...
                    <div style={{padding:'12px 14px',background:t.aiB,borderRadius:'16px 16px 16px 4px',border:`1px solid ${t.border}`}}>
                      <GenImg imageData={m._imageData} mimeType={m._mimeType} prompt={m._prompt} modelId={m._modelId||hfModel} t={t}/>
                      <div style={{fontSize:10,color:t.muted,marginTop:8,textAlign:'right'}}>{fmtTime(msg.created_at)}</div>
                    </div>
                  ):msg.type==='quiz'?(
                    ...
                    <div>
                      <div style={{padding:'10px 14px',background:msg.role==='user'?t.accent:t.aiB,color:msg.role==='user'?'#fff':t.txt,borderRadius:msg.role==='user'?'16px 16px 4px 16px':'16px 16px 16px 4px',border:msg.role==='assistant'?`1px solid ${isStar?'#f59e0b':t.border}`:'none',opacity:msg._tmp?.7:1}}>
                        {msg.role==='assistant'&&(isTyping||(!msg._tmp&&mdMode))
                          ? isTyping
                            ? <TypingText text={msg.content} isDark={t.isDark} markdown={mdMode}/>
          ...
          {err&&<div className="err-shake" style={{padding:'9px 13px',background:'#ff444418',border:'1px solid #ff444440',borderRadius:9,fontSize:13,color:'#ff6b6b',display:'flex',gap:8,wordBreak:'break-word',animation:'shake .4s ease'}}><span style={{flexShrink:0}}>⚠️</span><span>{err}</span></div>}
          <div ref={endRef}/>
        </div>
        ...
        <div style={{padding:'8px 12px 10px',background:t.iaBg,borderTop:`1px solid ${t.border}`,flexShrink:0}}>
          ...
          {isLoggedIn&&(
            <div style={{display:'flex',gap:5,marginBottom:8,flexWrap:'wrap',alignItems:'center'}}>
              {/* Live button - pulsing */}
              <button onClick={()=>setShowLive(true)} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:6,fontSize:12,fontWeight:600,background:'#ef4444',color:'#fff',border:'none',animation:'livePulse 2s infinite',cursor:'pointer',fontFamily:'inherit'}}>
                {Ic.live} 🔴 Live
              </button>
              <button onClick={()=>setImgMode(m=>m==='generate_image'?'chat':'generate_image')} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:6,fontSize:12,fontWeight:imgMode==='generate_image'?600:400,background:imgMode==='generate_image'?t.purple:t.btn,color:imgMode==='generate_image'?'#fff':t.muted,border:`1px solid ${imgMode==='generate_image'?t.purple:t.border}`,transition:'all .15s',cursor:'pointer',fontFamily:'inherit'}}>
                {Ic.magic}{imgMode==='generate_image'?'🎨 HF Obrázek':'AI Obrázek'}
              </button>
              <button onClick={()=>setImgMode(m=>m==='image_search'?'chat':'image_search')} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:6,fontSize:12,fontWeight:imgMode==='image_search'?600:400,background:imgMode==='image_search'?t.accent:t.btn,color:imgMode==='image_search'?'#fff':t.muted,border:`1px solid ${imgMode==='image_search'?t.accent:t.border}`,transition:'all .15s',cursor:'pointer',fontFamily:'inherit'}}>
                {Ic.imgSrch}{imgMode==='image_search'?'🔍 Fotky':'Fotky'}
              </button>
              <button onClick={()=>setQuizMode(m=>!m)} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:6,fontSize:12,fontWeight:quizMode?600:400,background:quizMode?'#f59e0b':t.btn,color:quizMode?'#fff':t.muted,border:`1px solid ${quizMode?'#f59e0b':t.border}`,transition:'all .15s',cursor:'pointer',fontFamily:'inherit'}}>
          ...
          {imgMode==='generate_image'&&isLoggedIn&&(
            <div style={{marginBottom:8}}>
              <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:5}}>
                {HF_MODELS.map(m=>(
                  <button key={m.id} onClick={()=>setHfModel(m.id)} style={{padding:'4px 9px',borderRadius:6,background:hfModel===m.id?t.purple:t.btn,color:hfModel===m.id?'#fff':t.muted,fontSize:11,border:`1px solid ${hfModel===m.id?t.purple:t.border}`,cursor:'pointer',fontFamily:'inherit',transition:'all .15s',fontWeight:hfModel===m.id?600:400}}>
                    {m.name}
                  </button>
                ))}
              </div>
              <div style={{fontSize:10,color:t.muted}}>⚠️ Vyžaduje HF_API_KEY v Supabase Secrets · první generování trvá 30–60s (model se načítá)</div>
            </div>
          )}
          ...
          {quizMode&&(
            <div style={{marginBottom:8,padding:'12px',background:t.tag,borderRadius:10,border:`1px solid #f59e0b44`,animation:'fadeIn .2s ease'}}>
              <div style={{display:'flex',gap:8,marginBottom:10}}>
                <input value={quizTopic} onChange={e=>setQuizTopic(e.target.value)} onKeyDown={e=>e.key==='Enter'&&sendQuiz()} placeholder="Téma kvízu (např. Fyzika, AI, Praha, Programování…)"
                  style={{flex:1,padding:'8px 12px',background:t.inBg,color:t.txt,border:`1.5px solid #f59e0b`,borderRadius:8,fontSize:13,outline:'none',fontFamily:'inherit'}}/>
              </div>
              <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
                <div style={{display:'flex',alignItems:'center',gap:6}}>
                  <span style={{fontSize:12,color:t.muted,flexShrink:0}}>Počet otázek:</span>
                  <div style={{display:'flex',gap:4'}}>
                    {[1,3,5,10,15,20].map(n=>(
                      <button key={n} onClick={()=>setQuizCount(n)} style={{padding:'4px 8px',borderRadius:6,background:quizCount===n?'#f59e0b':t.btn,color:quizCount===n?'#fff':t.muted,fontSize:12,border:'none',cursor:'pointer',fontFamily:'inherit',fontWeight:quizCount===n?700:400,minWidth:28,transition:'all .15s'}}>
                        {n}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{display:'flex',alignItems:'center',gap:6}}>
                  ...
                  {[['easy','🟢 Lehká'],['medium','🟡 Střední'],['hard','🔴 Těžká']].map(([v,l])=>(
                    <button key={v} onClick={()=>setQuizDiff(v)} style={{padding:'4px 9px',borderRadius:6,background:quizDiff===v?'#f59e0b':t.btn,color:quizDiff===v?'#fff':t.muted,fontSize:12,border:'none',cursor:'pointer',fontFamily:'inherit',fontWeight:quizDiff===v?700:400,transition:'all .15s'}}>
                      {l}
                    </button>
                  ))}
                </div>
                <button onClick={sendQuiz} disabled={!quizTopic.trim()||loading}
                  style={{padding:'6px 16px',borderRadius:8,background:'#f59e0b',color:'#fff',fontSize:13,fontWeight:600,border:'none',cursor:'pointer',fontFamily:'inherit',opacity:!quizTopic.trim()?0.5:1,marginLeft:'auto'}}>
                  Start 🎓
                </button>
              </div>
            </div>
          ...
          <div style={{display:'flex',alignItems:'flex-end',gap:6,padding:'9px 11px',background:t.inBg,border:`1.5px solid ${imgMode!=='chat'?modeColor:thinking?t.purple:t.inBrd}`,borderRadius:14,transition:'border-color .2s'}}>
            ...
            <div style={{display:'flex',gap:4,alignItems:'center',flexShrink:0}}>
              ...
              <button onClick={send} disabled={!canSend}
                style={{width:34,height:34,borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center',background:canSend?(imgMode==='generate_image'||thinking?t.purple:t.accent):t.btn,color:canSend?'#fff':t.muted,transition:'all .15s',flexShrink:0,border:'none',cursor:'pointer'}}>
                {Ic.send}
              </button>
            </div>
          </div>
          ...
          {/* Beta disclaimer */}
          <div style={{fontSize:10,color:t.muted,textAlign:'center',marginTop:6,lineHeight:1.4}}>
            <span style={{background:'#f59e0b22',color:'#f59e0b',padding:'1px 5px',borderRadius:3,fontWeight:600,marginRight:5}}>BETA</span>
            AI Asistent může dělat chyby — vždy ověřte důležité informace
            {isLoggedIn?' · Paměť + Historie uložena':' · Přihlaste se pro plné funkce'}
          </div>
        </div>
      </main>
