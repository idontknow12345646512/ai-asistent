...
import {
  ...
  CONV_COLORS, IMG_MODELS, AI_MODELS, PERSONAS, QUIZ_COUNTS, QUIZ_DIFFS,
  MEM_CATEGORIES, THEMES, THEME_LIST, Ic, LumiAvatar,
...
function GenImg({ imageData, mimeType, prompt, modelId, t }) {
  ...
  const modelName = IMG_MODELS.find(m=>m.id===modelId)?.name || modelId || 'HF'
  const dl = () => { const a=document.createElement('a');a.href=src;a.download=`lumi-${Date.now()}.png`;a.click() }
  return (
    <div>
...
function QuizCard({ questions, t }) {
  ...
  if (!questions?.length) return (
    <div style={{padding:'12px 14px',background:t.aiB,borderRadius:'16px 16px 16px 4px',border:`1px solid ${t.border}`,fontSize:13,color:t.muted}}>Kvíz nemá žádné otázky.</div>
  )
  ...
  if (done) {
    ...
    return (
      <div style={{padding:'20px',background:t.aiB,borderRadius:'16px 16px 16px 4px',border:`1px solid ${t.border}`,textAlign:'center'}}>
        ...
        <div style={{fontSize:13,color:t.muted,marginTop:12,marginBottom:18}}>
          {pct>=80?'Výborně! Skvělý výkon!':pct>=60?'Dobrá práce!':pct>=40?'Slušný výsledek!':'Příště to půjde lépe!'}
        </div>
...
function VoiceBtn({ t, onTranscript }) {
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:8}}>
      ...
      {txt&&<span style={{fontSize:12,color:t.muted,fontStyle:'italic',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:120}}>„{txt}"</span>}
    </div>
  )
}
...
function ToolDropdown({ t, label, icon, children, accent, active }) {
  ...
  return (
    <div ref={ref} style={{position:'relative'}}>
      ...
      {open&&(
        <div style={{position:'absolute',bottom:'calc(100% + 6px)',left:0,minWidth:220,background:t.modal,border:`1px solid ${t.border}`,borderRadius:12,padding:6,zIndex:40,boxShadow:`0 8px 32px rgba(0,0,0,.4)`,animation:'fadeIn .15s ease'}}>
          {children}
  ...
  return (
    <>
      ...
      <div onClick={e=>e.stopPropagation()}
        style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',zIndex:59,width:'min(420px,96vw)',background:t.modal,border:`1px solid ${t.border}`,borderRadius:16,padding:24,fontFamily:"'DM Sans',sans-serif"}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
          <h3 style={{fontSize:15,fontWeight:600,color:t.txt}}>➕ Přidat do paměti</h3>
          <button onClick={onClose} style={{color:t.muted,display:'flex',padding:3,background:'none',border:'none',cursor:'pointer'}}>{Ic.x}</button>
        </div>
        <div style={{fontSize:11,fontWeight:600,color:t.muted,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:8}}>Co chcete, aby Lumi věděl?</div>
        <textarea value={content} onChange={e=>setContent(e.target.value)} rows={4}
          placeholder="Např. Jmenuji se Radek a jsem student programování…"
...
function SettingsModal({ t, themeName, setThemeName, sysPmt, setSysPmt, onClose, isLoggedIn, userId, memory, setMemory, aiModel, setAiModel, onAddMemory }) {
  ...
  useEffect(()=>{
    if(tab==='memory'&&isLoggedIn) {
      supabase.from('user_memory').select('*').eq('user_id',userId).order('created_at',{ascending:false}).limit(50)
  ...
  return (
    <>
      ...
      <div onClick={e=>e.stopPropagation()}
        style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',zIndex:50,width:'min(580px,96vw)',maxHeight:'90vh',display:'flex',flexDirection:'column',background:t.modal,border:`1px solid ${t.border}`,borderRadius:18,fontFamily:"'DM Sans',sans-serif",overflow:'hidden'}}>
        <div style={{padding:'18px 20px 0',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <LumiAvatar size={32} gradient={[t.gradA,t.gradB]}/>
            <h2 style={{fontSize:16,fontWeight:600,color:t.txt}}>Nastavení</h2>
          </div>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 20px 20px'}}>
          ...
          {tab==='appearance'&&(
            <>
              <div style={{fontSize:11,fontWeight:600,color:t.muted,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:12}}>Barevné téma (12 možností)</div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:7}}>
                {THEME_LIST.map(th=>(
                  <button key={th.id} onClick={()=>setThemeName(th.id)}
                    style={{padding:'10px 6px',borderRadius:9,border:`2px solid ${themeName===th.id?t.accent:t.border}`,background:themeName===th.id?t.accent+'22':t.btn,color:themeName===th.id?t.accent:t.muted,fontSize:11,fontWeight:themeName===th.id?600:400,cursor:'pointer',fontFamily:'inherit',textAlign:'center',transition:'all .15s'}}>
                    <div style={{fontSize:18,marginBottom:3}}>{th.icon}</div>
                    {th.label}
                  </button>
                ))}
              </div>
          ...
          {tab==='model'&&(
            <>
              ...
              <div style={{padding:'12px 14px',borderRadius:10,background:t.tag,border:`1px solid ${t.border}`,fontSize:12,color:t.muted,lineHeight:1.6}}>
                💡 <strong style={{color:t.txt}}>Gemma 3 27B</strong> je největší open-source model od Google. Funguje přes Gemini API a nepodporuje nativní system prompt — je vložen jako první zpráva konverzace. <strong style={{color:t.txt}}>Deep Thinking</strong> funguje pouze s Gemini 2.5.
              </div>
            </>
          )}
            ...
            <>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                <div style={{fontSize:11,fontWeight:600,color:t.muted,textTransform:'uppercase',letterSpacing:'.08em'}}>
                  Paměť {memList.length>0&&`(${memList.length})`}
                </div>
                <div style={{display:'flex',gap:6}}>
                  <button onClick={()=>{onAddMemory();onClose()}}
                    style={{display:'flex',alignItems:'center',gap:4,padding:'5px 10px',borderRadius:7,background:t.accent,color:'#fff',fontSize:12,fontWeight:600,border:'none',cursor:'pointer',fontFamily:'inherit'}}>
                    {Ic.addMem} Přidat
                  </button>
              ...
              {isLoggedIn&&memList.length===0&&(
                <div style={{textAlign:'center',padding:'24px 0',color:t.muted,fontSize:13}}>
                  <div style={{fontSize:32,marginBottom:8}}>🧠</div>
                  Paměť je prázdná. Přidejte informace o sobě aby vás Lumi lépe znal.
                </div>
              )}
              {memList.map(m=>(
                <div key={m.id} style={{display:'flex',alignItems:'flex-start',gap:8,padding:'9px 12px',borderRadius:9,background:t.tag,border:`1px solid ${t.border}`,marginBottom:7}}>
                  <div style={{flexShrink:0,marginTop:1}}>
                    <span style={{fontSize:10,color:t.muted,background:t.btn,padding:'2px 6px',borderRadius:4}}>{MEM_CATEGORIES.find(c=>c.id===m.category)?.label||m.category}</span>
                    {m.source==='manual'&&<span style={{fontSize:9,color:t.accent,background:t.accent+'22',padding:'1px 5px',borderRadius:3,marginLeft:4}}>ručně</span>}
                  </div>
          ...
          {tab==='about'&&(
            <div style={{fontSize:13,color:t.muted,lineHeight:1.7}}>
              ...
              {[['🤖 Chat','Gemini 3.1 Flash Lite / Flash'],['💭 Deep Thinking','Gemini 2.5 Flash/Pro'],['🔶 Gemma','Gemma 3 12B / 27B'],['🔴 Live','Gemini 2.0 Flash Live'],['🎨 AI Obrázky','HuggingFace (SD 3.5, FLUX.1)'],['📷 Fotografie','Unsplash API'],['🎙️ Hlas','Web Speech API'],['🧠 Paměť','Supabase PostgreSQL'],['🔒 Auth','Supabase Auth']].map(([k,v])=>(
...
export default function Chat({ session }) {
  ...
  const sendQuiz=async()=>{
    ...
    try{
      ...
      if(isLocal)setConvs(p=>p.map(c=>c.id!==cid?c:{...c,messages:[...(c.messages??[]),aMsg]}))
        ...
        // Uložit kvíz správně: type='quiz', data v image_url, content='🎓 Kvíz'
  ...
  // Přidat paměť manuálně
  const addMemoryManual=async(content,category)=>{
    if(!isLoggedIn||!content) return
    try{await callEdge('save_memory',{content,category,source:'manual'},token)}catch{}
  }
  ...
  const addNewAnim=id=>{
    setNewIds(s=>{const n=new Set(s);n.add(id);setTimeout(()=>setNewIds(s2=>{const n2=new Set(s2);n2.delete(id);return n2}),800);return n})
  }
  ...
  const send=useCallback(async()=>{
    ...
    atts.forEach(a=>{
      ...
      else api.push({type:'text',text:`[Soubor: ${a.name} (${a.type})]`})
    })
  ...
  const phs={chat:thinking?'💭 Deep Thinking (Gemini 2.5)…':'Napište zprávu… (Enter = odeslat)',image_search:'🔍 Popište co hledáte…',generate_image:'🎨 Popište obrázek…'}
          ...
          <div style={{padding:'13px 12px',borderBottom:`1px solid ${t.border}`,display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
            ...
            <div style={{display:'flex',gap:4}}>
              ...
              <button onClick={newConv}
                style={{background:t.accent,color:'#fff',borderRadius:7,padding:'5px 9px',display:'flex',alignItems:'center'}}>{Ic.plus}</button>
            </div>
          </div>
      ...
      <main style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
        ...
        <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 14px',height:52,background:t.hdr,borderBottom:`1px solid ${t.border}`,backdropFilter:'blur(12px)',flexShrink:0,gap:8}}>
          <div style={{display:'flex',alignItems:'center',gap:8,minWidth:0}}>
            ...
            {thinking&&<span style={{fontSize:10,background:t.purple+'22',color:t.purple,padding:'2px 7px',borderRadius:4,flexShrink:0,fontWeight:600,animation:'thinkPulse 1.5s infinite'}}>💭 ON</span>}
          </div>
          <div style={{display:'flex',gap:3,alignItems:'center',flexShrink:0}}>
            {/* Přidat do paměti */}
            {isLoggedIn&&(
              <button className="ib" onClick={()=>setShowAddMem(true)} title="Přidat do paměti Lumi"
            ...
            <button className="ib" onClick={()=>setThemeName(n=>{const ks=Object.keys(THEMES);return ks[(ks.indexOf(n)+1)%ks.length]})} title="Změnit téma"
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 14px',display:'flex',flexDirection:'column',gap:12}}>
          {displayMsgs.length===0&&!loading&&(
            <div style={{textAlign:'center',marginTop:'7vh',padding:'0 16px',animation:'fadeIn .5s ease'}}>
              ...
              <div style={{fontSize:22,fontWeight:700,marginBottom:6,color:t.txt}}>
                {showStarred?'Žádné oblíbené zprávy':`Ahoj! Jsem ${APP_NAME}.`}
              </div>
              {!showStarred&&(
                <>
                  <div style={{fontSize:13,color:t.muted,marginBottom:20,lineHeight:1.6}}>
                    {isLoggedIn?'Chat · AI Obrázky (HF) · Fotografie · Kvízy · Live · Hlas':'Začněte psát — přihlášení není potřeba'}
                  </div>
                  ...
                  {msg._atts?.length>0&&(
                    <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:6,justifyContent:'flex-end'}}>
                      {msg._atts.map(a=>a.preview
                        ...
                        :<div key={a.id} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 9px',background:t.pill,borderRadius:7,fontSize:12,color:t.txt}}>{a.type.includes('pdf')?Ic.pdf:Ic.file}{a.name.length>14?a.name.slice(0,12)+'…':a.name}</div>
                      )}
                    </div>
                  )}
                  {/* Message */}
        ...
        <div style={{padding:'8px 12px 10px',background:t.iaBg,borderTop:`1px solid ${t.border}`,flexShrink:0}}>
            ...
            <div style={{display:'flex',gap:5,marginBottom:8,flexWrap:'wrap',alignItems:'center'}}>
              ...
              <ToolDropdown t={t} label="Nástroje" icon={Ic.wand} active={imgMode!=='chat'||quizMode}>
                <div style={{padding:'6px 0'}}>
                  {[{id:'chat',icon:'💬',label:'Chat',color:t.accent},{id:'generate_image',icon:'🎨',label:'AI Obrázek',color:t.purple},{id:'image_search',icon:'📷',label:'Hledat fotografie',color:t.green}].map(item=>(
                    <button key={item.id} onClick={()=>setImgMode(item.id)}
                      ...
                      <span style={{fontWeight:imgMode===item.id?600:400}}>{item.label}</span>
                  ...
                  <button onClick={()=>setQuizMode(m=>!m)}
                    ...
                    <span style={{fontWeight:quizMode?600:400}}>Kvíz</span>
              ...
              <ToolDropdown t={t} label="Model" icon={Ic.model} accent={t.purple} active={aiModel!=='default'||thinking}>
                <div style={{padding:'6px 0'}}>
                  ...
                  {imgMode==='generate_image'&&(
                    <>
                      <div style={{margin:'4px 12px',borderTop:`1px solid ${t.border}`}}/>
                      <div style={{fontSize:10,fontWeight:600,color:t.muted,textTransform:'uppercase',letterSpacing:'.08em',padding:'4px 12px 6px'}}>AI Obrázky (HuggingFace)</div>
                      {IMG_MODELS.map(m=>(
                      ...
                      <div style={{margin:'4px 12px',padding:'4px 0',fontSize:10,color:t.muted,textAlign:'center'}}>
                        Vyžaduje HF_API_KEY (Read token) v Supabase Secrets
                      </div>
                    </>
                  )}
                </div>
              ...
              {imgMode!=='chat'&&<span style={{fontSize:11,padding:'3px 8px',borderRadius:5,background:imgMode==='generate_image'?t.purple+'22':'rgba(34,197,94,.15)',color:imgMode==='generate_image'?t.purple:t.green,fontWeight:600}}>
                {imgMode==='generate_image'?`🎨 ${IMG_MODELS.find(m=>m.id===imgModel)?.name||'HF'}`:' 📷 Fotky'}
              </span>}
            </div>
          )}
          ...
          {quizMode&&(
            <div style={{marginBottom:8,padding:'12px',background:t.tag,borderRadius:10,border:`1px solid #f59e0b44`,animation:'fadeIn .2s ease'}}>
              <input value={quizTopic} onChange={e=>setQuizTopic(e.target.value)} onKeyDown={e=>e.key==='Enter'&&sendQuiz()}
                placeholder="Téma kvízu (Fyzika, AI, Praha, Programování…)"
              ...
              {atts.map(a=>(
                <div key={a.id} style={{position:'relative',animation:'fadeIn .2s ease'}}>
                  {a.preview?<img src={a.preview} alt={a.name} style={{height:46,width:46,objectFit:'cover',borderRadius:7,border:`1px solid ${t.border}`,display:'block'}}/>
                    :<div style={{display:'flex',alignItems:'center',gap:5,padding:'4px 9px',background:t.pill,borderRadius:7,fontSize:12,color:t.txt,maxWidth:140}}>
                      {a.type.includes('pdf')?Ic.pdf:a.type.includes('image')?Ic.img:Ic.file}
                      <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.name.length>16?a.name.slice(0,14)+'…':a.name}</span>
                    </div>}
          ...
          <div style={{display:'flex',alignItems:'flex-end',gap:6,padding:'9px 11px',background:t.inBg,border:`1.5px solid ${imgMode!=='chat'?modeColor:thinking?t.purple:t.inBrd}`,borderRadius:14,transition:'border-color .2s'}}>
            ...
            <div style={{display:'flex',gap:4,alignItems:'center',flexShrink:0}}>
              ...
              <button className="ib" onClick={()=>fileRef.current.click()} style={{color:t.muted,display:'flex',padding:5}} title="Přidat soubor (obrázek, PDF, txt…)">{Ic.clip}</button>
          ...
          {/* Accept all file types including PDF */}
