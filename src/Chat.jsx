...
const HF_MODELS = [
  { id:'black-forest-labs/FLUX.1-schnell',         name:'FLUX Schnell ⚡', desc:'Nejrychlejší, 4 kroky' },
  { id:'stabilityai/stable-diffusion-xl-base-1.0', name:'SDXL 🎨',         desc:'Vysoká kvalita' },
  { id:'runwayml/stable-diffusion-v1-5',            name:'SD 1.5 🖼️',       desc:'Klasický model' },
  { id:'Lykon/dreamshaper-8',                       name:'DreamShaper ✨',   desc:'Stylizovaný' },
  { id:'prompthero/openjourney-v4',                 name:'OpenJourney 🚀',  desc:'Midjourney styl' },
]

const AI_MODELS = [
  { id:'default',     name:'Gemini Auto ✦',  desc:'Automaticky nejlepší dostupný' },
  { id:'gemma-3-12b', name:'Gemma 3 12B 🔶', desc:'Open-source Google, rychlý' },
  { id:'gemma-3-27b', name:'Gemma 3 27B 🔷', desc:'Open-source Google, nejsilnější' },
]
...
const PERSONAS = [
  ...
  { label:'Kreativní spisovatel',   val:'Jsi kreativní spisovatel. Pomáháš s texty, příběhy, básněmi a scénáři s bohatým jazykem.' },
  { label:'Lektor / učitel',        val:'Jsi trpělivý lektor. Vysvětluješ jednoduše, používáš příklady a analogie. Přizpůsobuješ se tempu studenta.' },
  { label:'Sokrates 🏛️',            val:'Jsi sokratovský filosof. Odpovídáš otázkami, vedeš dialog a pomáháš hledat pravdu skrze rozhovor.' },
]

const QUIZ_COUNTS = [1,3,5,10,15,20]
const QUIZ_DIFFS  = [['easy','🟢 Lehká'],['medium','🟡 Střední'],['hard','🔴 Těžká']]
...
function renderMD(text, isDark) {
  if (!text) return ''
  const cb   = isDark ? '#0d1117' : '#f6f8fa'
  const cbrd = isDark ? '#30363d' : '#d0d7de'
  const ib   = isDark ? 'rgba(110,118,129,0.2)' : 'rgba(175,184,193,0.2)'
  return text
    .replace(/```(\w*)\n?([\s\S]*?)```/g, (_, lang, code) => {
      const lbl = lang
        ? `<div style="display:flex;justify-content:space-between;padding:5px 12px;background:${isDark?'#161b22':'#eaeef2'};border-bottom:1px solid ${cbrd};font-size:11px;color:${isDark?'#8b949e':'#57606a'};font-family:monospace;border-radius:8px 8px 0 0"><span style="font-weight:600">${lang}</span><span>code</span></div>`
        : ''
...
const TH = {
  ...
  light:    { bg:'#f4f6fb', side:'#ffffff', hdr:'rgba(244,246,251,.96)', txt:'#1a1d2a', muted:'#7a849a', border:'#e2e6f0', accent:'#4c6ef5', purple:'#9333ea', green:'#16a34a', active:'#eef1ff', aiB:'#ffffff', inBg:'#ffffff', iaBg:'#f4f6fb', inBrd:'#d8dde8', btn:'#edf0f7', modal:'#ffffff', pill:'#edf0f7', ua:'#8898b0', scrl:'#c8cdd8', card:'#edf0f7', tag:'#eef1ff', success:'#f0fff4', succ:'#276749', danger:'#dc2626', isDark:false },
...
function TypingText({ text, isDark, useMarkdown, onDone }) {
  ...
  useEffect(() => {
    ...
    let i = 0
    const speed = text.length > 600 ? 3 : text.length > 200 ? 5 : 8
    const id = setInterval(() => {
      i += speed
      if (i >= text.length) {
        setShown(text); setFinished(true)
        clearInterval(id)
        onDone && onDone()
      } else {
  ...
  return (
    <div>
      ...
      <div style={{ position:'relative', display:'inline-block', maxWidth:'100%' }}>
        ...
        <button onClick={dl}
          style={{ position:'absolute', top:8, right:8, display:'flex', alignItems:'center', gap:5, padding:'5px 10px', borderRadius:7, background:'rgba(0,0,0,.65)', color:'#fff', fontSize:11, border:'none', cursor:'pointer', backdropFilter:'blur(4px)', fontFamily:'inherit' }}>
          {Ic.dl} Stáhnout
        </button>
      </div>
    ...
    <div style={{ padding:'14px 16px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
        {q.options?.map((opt, i) => {
          ...
          if (sel !== undefined) {
            if (isC)        { bg='rgba(34,197,94,.15)'; clr=t.succ; brd=t.succ }
            else if (isSel) { bg='rgba(239,68,68,.15)'; clr='#fca5a5'; brd='#f87171' }
          }
          return (
            <button key={i}
              onClick={() => sel === undefined && setAnswers(p => ({ ...p, [cur]: i }))}
...
// ── Char counter ──────────────────────────────────────────────────────────────
function CharCount({ count, max = 2000, t }) {
  if (count < 100) return null
  const pct = count / max
  const clr = pct > .9 ? t.danger : pct > .7 ? '#f59e0b' : t.muted
  return <span style={{ fontSize:10, color:clr, flexShrink:0 }}>{count}/{max}</span>
}
  ...
  return (
    <>
      ...
      <div onClick={e => e.stopPropagation()}
        style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:50, width:'min(540px,96vw)', maxHeight:'88vh', display:'flex', flexDirection:'column', background:t.modal, border:`1px solid ${t.border}`, borderRadius:18, fontFamily:"'DM Sans',sans-serif", overflow:'hidden' }}>
          ...
          {tab === 'appearance' && (
            <>
              ...
              <div style={{ display:'flex', gap:8, marginBottom:20 }}>
                {[{id:'dark',label:'Tmavý',icon:'🌙'},{id:'light',label:'Světlý',icon:'☀️'},{id:'midnight',label:'Midnight',icon:'🌌'}].map(th => (
                  <button key={th.id} onClick={() => setThemeName(th.id)}
                    style={{ flex:1, padding:'12px 8px', borderRadius:10, border:`2px solid ${themeName===th.id?t.accent:t.border}`, background:themeName===th.id?t.accent+'22':t.btn, color:themeName===th.id?t.accent:t.muted, fontSize:12, fontWeight:themeName===th.id?600:400, cursor:'pointer', fontFamily:'inherit', textAlign:'center', transition:'all .15s' }}>
          ...
          {tab === 'model' && (
            <>
              ...
              <div style={{ padding:'12px 14px', borderRadius:10, background:t.tag, border:`1px solid ${t.border}`, fontSize:12, color:t.muted, lineHeight:1.6 }}>
                💡 <strong style={{ color:t.txt }}>Gemma</strong> jsou open-source modely od Google. Gemma 3 27B je největší veřejně dostupný model od Google.
              </div>
            </>
          )}
          ...
          {tab === 'about' && (
            <div style={{ fontSize:13, color:t.muted, lineHeight:1.7 }}>
              ...
              {[
                ...
                ['💭 Deep Thinking', 'Gemini 2.5 Flash/Pro'],
                ['🔶 Gemma',         'Gemma 3 12B / 27B'],
                ['🔴 Live',          'Gemini 2.0 Flash Live'],
                ['🎨 AI Obrázky',    'HuggingFace FLUX / SD'],
                ['📷 Fotografie',    'Unsplash API'],
                ['🎙️ Hlas',          'Web Speech API'],
                ['🧠 Paměť',         'Supabase PostgreSQL'],
                ['🔒 Auth',          'Supabase Auth'],
              ].map(([k, v]) => (
...
export default function Chat({ session }) {
  ...
  const [msgCount,   setMsgCount]   = useState(0)  // stats
  ...
  const sendQuiz = async () => {
    ...
    try {
      ...
      if (isLocal) setConvs(p => p.map(c => c.id !== cid ? c : { ...c, messages:[...(c.messages ?? []), aMsg] }))
        ...
        await saveMsg(cid, 'assistant', JSON.stringify(qs), 'text', null)
  ...
  const currentAIModel = AI_MODELS.find(m => m.id === aiModel)?.name || 'Gemini'
  ...
  function getImgData(msg) {
    ...
    if (msg.image_url) {
      try {
        ...
        return { ...msg, _images:Array.isArray(p)?p:undefined, _imageData:p.imageData, _mimeType:p.mimeType, _prompt:p.prompt, _modelId:p.modelId, _query:msg.content }
      } catch { return msg }
    }
    return msg
  }
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hbtn{transition:all .15s}.hbtn:hover{background:${t.active}!important;opacity:1!important}
      ...
      {sideOpen && (
        <aside className="sidebar" style={{ width:272, background:t.side, borderRight:`1px solid ${t.border}`, display:'flex', flexDirection:'column', flexShrink:0 }}>
          ...
          <div style={{ padding:'13px 12px', borderBottom:`1px solid ${t.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <div style={{ width:28, height:28, borderRadius:8, background:`linear-gradient(135deg,${t.accent},${t.purple})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:700, color:'#fff', flexShrink:0 }}>A</div>
          ...
          <div style={{ padding:'10px 11px', borderTop:`1px solid ${t.border}` }}>
            {isLoggedIn ? (
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <div style={{ width:32, height:32, borderRadius:9, background:`linear-gradient(135deg,${t.accent}44,${t.purple}44)`, border:`1px solid ${t.accent}55`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:700, color:t.accent, flexShrink:0 }}>{userInitial}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:10, color:t.muted }}>Přihlášen · {isLoggedIn ? `${msgs.length} zpráv` : ''}</div>
                </div>
      ...
      <main style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        <header style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', height:52, background:t.hdr, borderBottom:`1px solid ${t.border}`, backdropFilter:'blur(12px)', flexShrink:0, gap:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
            ...
            {aiModel !== 'default' && <span style={{ fontSize:10, background:t.purple+'22', color:t.purple, padding:'2px 7px', borderRadius:4, flexShrink:0, fontWeight:600 }}>{currentAIModel}</span>}
          </div>
          <div style={{ display:'flex', gap:3, alignItems:'center', flexShrink:0 }}>
            {[
              { icon: showStarred ? Ic.starF : Ic.star, action: () => setShowStarred(s => !s), active: showStarred, color: '#f59e0b', title:'Oblíbené' },
              { icon: Ic.export, action: exportChat, active: false, color: null, title:'Export chatu' },
              { icon: <span style={{fontSize:11,fontWeight:600}}>MD</span>, action: () => setMdMode(m => !m), active: mdMode, color: null, title:'Markdown' },
            ].map(({ icon, action, active, color, title }, i) => (
              <button key={i} onClick={action} title={title}
                style={{ display:'flex', alignItems:'center', justifyContent:'center', padding:'5px 8px', borderRadius:7, background:active?(color?color+'22':t.accent+'22'):t.btn, color:active?(color||t.accent):t.muted, border:`1px solid ${active?(color||t.accent):t.border}`, transition:'all .15s', cursor:'pointer' }}>
                {icon}
              </button>
            ))}
            {isLoggedIn && (
              <button onClick={() => setThinking(x => !x)} title="Hluboké přemýšlení (Gemini 2.5)"
                style={{ display:'flex', alignItems:'center', gap:3, padding:'5px 8px', borderRadius:7, background:thinking?t.purple+'33':t.btn, color:thinking?t.purple:t.muted, border:`1px solid ${thinking?t.purple:t.border}`, fontSize:11, transition:'all .15s', cursor:'pointer' }}>
                {Ic.brain}{thinking && <span style={{ animation:'thinkPulse 1.5s infinite' }}> ON</span>}
              </button>
            )}
            <button onClick={() => setShowSet(true)} title="Nastavení"
              style={{ display:'flex', padding:'5px 8px', borderRadius:7, background:t.btn, color:t.muted, border:`1px solid ${t.border}`, cursor:'pointer' }}>
              {Ic.gear}
            </button>
            <button onClick={() => setThemeName(n => n==='dark'?'light':n==='light'?'midnight':'dark')} title="Změnit téma"
              style={{ display:'flex', padding:'5px 8px', borderRadius:7, background:t.btn, color:t.muted, border:`1px solid ${t.border}`, cursor:'pointer' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px 14px', display:'flex', flexDirection:'column', gap:12 }}>
          {displayMsgs.length === 0 && !loading && (
            <div style={{ textAlign:'center', marginTop:'7vh', padding:'0 16px', animation:'fadeIn .5s ease' }}>
              <div style={{ width:56, height:56, borderRadius:16, background:`linear-gradient(135deg,${t.accent}33,${t.purple}33)`, border:`1.5px solid ${t.accent}44`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, margin:'0 auto 16px', color:t.accent }}>✦</div>
              <div style={{ fontSize:21, fontWeight:600, marginBottom:8 }}>
                {showStarred ? 'Žádné oblíbené zprávy' : 'Jak Vám mohu pomoci?'}
              </div>
              {!showStarred && (
                <>
                  <div style={{ fontSize:13, color:t.muted, marginBottom:20, lineHeight:1.6 }}>
                    {isLoggedIn
                      ? 'Chat · HF AI Obrázky · Fotografie · Kvízy · Live · Hlasový vstup · Gemma 3'
                      : 'Začněte psát — přihlášení není potřeba'}
                  </div>
                  <div style={{ display:'flex', gap:7, justifyContent:'center', flexWrap:'wrap', maxWidth:520, margin:'0 auto' }}>
                    {(isLoggedIn
                      ? ['Jak funguje kvantové počítání?', 'Najdi fotky Prahy', 'Vygeneruj: forest at sunset', 'Kvíz o historii ČR']
          ...
          {displayMsgs.map(msg => {
            ...
            return (
              <div key={msg.id} className={isNew ? 'msg-new' : 'msg-old'}
                ...
                {/* AI avatar */}
                {msg.role === 'assistant' && (
                  <div style={{ width:28, height:28, borderRadius:8, background:`linear-gradient(135deg,${t.accent},${t.purple})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, color:'#fff', flexShrink:0, marginTop:2 }}>A</div>
                )}
          ...
          {loading && (
            <div style={{ display:'flex', gap:8, alignItems:'flex-start', animation:'fadeIn .2s ease' }}>
              <div style={{ width:28, height:28, borderRadius:8, background:`linear-gradient(135deg,${t.accent},${t.purple})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, color:'#fff', flexShrink:0 }}>A</div>
              <div style={{ padding:'12px 16px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
                {ltxt[imgMode]
                  ? <span style={{ fontSize:13, color:t.muted }}>{ltxt[imgMode]}</span>
                  : thinking
                    ? <span style={{ fontSize:13, color:t.purple, animation:'thinkPulse 1.5s infinite' }}>💭 Gemini 2.5 přemýšlí…</span>
          ...
          {/* Error */}
          {err && (
            <div className="err-shake" style={{ padding:'9px 13px', background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.3)', borderRadius:9, fontSize:13, color:'#fca5a5', display:'flex', gap:8, wordBreak:'break-word' }}>
          ...
          {isLoggedIn && (
            <div style={{ display:'flex', gap:5, marginBottom:8, flexWrap:'wrap', alignItems:'center' }}>
              <button onClick={() => setShowLive(true)}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:6, fontSize:12, fontWeight:600, background:'#ef4444', color:'#fff', border:'none', animation:'livePulse 2s infinite', cursor:'pointer', fontFamily:'inherit' }}>
                {Ic.live} 🔴 Live
              </button>
              <button onClick={() => setImgMode(m => m==='generate_image'?'chat':'generate_image')}
              ...
              <button onClick={() => setImgMode(m => m==='image_search'?'chat':'image_search')}
              ...
              <button onClick={() => setQuizMode(m => !m)}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:6, fontSize:12, fontWeight:quizMode?600:400, background:quizMode?'#f59e0b':t.btn, color:quizMode?'#fff':t.muted, border:`1px solid ${quizMode?'#f59e0b':t.border}`, transition:'all .15s', cursor:'pointer', fontFamily:'inherit' }}>
          ...
          {imgMode === 'generate_image' && isLoggedIn && (
            <div style={{ marginBottom:8 }}>
              <div style={{ display:'flex', gap:4, flexWrap:'wrap', marginBottom:5 }}>
                {HF_MODELS.map(m => (
                  <button key={m.id} onClick={() => setHfModel(m.id)}
                    style={{ padding:'4px 9px', borderRadius:6, background:hfModel===m.id?t.purple:t.btn, color:hfModel===m.id?'#fff':t.muted, fontSize:11, border:`1px solid ${hfModel===m.id?t.purple:t.border}`, cursor:'pointer', fontFamily:'inherit', transition:'all .15s', fontWeight:hfModel===m.id?600:400 }}>
                    {m.name}
                  </button>
                ))}
              </div>
              <div style={{ fontSize:10, color:t.muted }}>⚠️ Vyžaduje HF_API_KEY (Read token) v Supabase Secrets · první generování ~30–60 s</div>
            </div>
          )}
