...
function detectImageSearch(text) {
  ...
  return ['najdi obrázk', 'vyhledej obrázk', 'ukaž obrázk', 'zobraz obrázk',
    ...
    'search image', 'show image', 'obrázek ', 'foto ', 'picture of', 'photo of'
...
function ImageSearchResults({ images, query, t }) {
  ...
  if (!images?.length) return (
    <div style={{ fontSize:13, color:t.muted }}>🔍 Žádné obrázky nenalezeny pro „{query}"</div>
  )
  return (
    <div>
      ...
      <div style={{ fontSize:10, color:t.muted, marginTop:8 }}>
        Fotografie z{' '}
        <a href="https://unsplash.com?utm_source=ai_asistent&utm_medium=referral" target="_blank" rel="noopener noreferrer"
          style={{ color:t.accent, textDecoration:'none', fontWeight:500 }}>Unsplash</a>
...
export default function Chat({ session }) {
  ...
  const [imgSearchMode, setImgSearchMode] = useState(false)
  ...
  const send = useCallback(async () => {
    ...
    const wantsImages = isLoggedIn && (imgSearchMode || detectImageSearch(userText))
    ...
    try {
      ...
      if (isLoggedIn) {
        ...
        if (!token) throw new Error('Nepodařilo se obnovit přihlášení. Odhlaste se a přihlaste znovu.')
        const res = await fetch(EDGE_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
          body: JSON.stringify({ messages: history, system: sysPmt, mode: wantsImages ? 'image_search' : 'chat' }),
        })
        if (!res.ok) { const t2 = await res.text(); let p; try { p = JSON.parse(t2) } catch { p = null }; throw new Error(p?.error || `HTTP ${res.status}`) }
      ...
      if (result.type === 'image_search') {
        aMsg = {
          id: uid(), role: 'assistant', type: 'image_search',
          content: `Nalezeno ${result.images?.length ?? 0} obrázků pro „${result.query}"`,
          _images: result.images, _query: result.query,
          image_url: JSON.stringify(result.images),
      ...
      if (isLocal) {
        ...
        await saveDbMsg(convId, 'assistant', aMsg.content, aMsg.type, aMsg._images ?? null)
  ...
  }, [input, atts, loading, activeConv, msgs, isLoggedIn, imgSearchMode, sysPmt]) // eslint-disable-line
  ...
  function getImages(msg) {
    if (msg._images) return msg._images
    if (msg.image_url) { try { return JSON.parse(msg.image_url) } catch { return [] } }
    return []
  }
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 16px', display:'flex', flexDirection:'column', gap:12 }}>
          {displayMsgs.length === 0 && !loading && (
            <div style={{ textAlign:'center', marginTop:'9vh' }}>
              ...
              <div style={{ fontSize:13, color:t.muted, marginBottom:14 }}>
                {isLoggedIn ? 'Napište dotaz nebo vyhledejte fotografie 📷' : 'Začněte psát — přihlášení není potřeba'}
              </div>
              {isLoggedIn && (
                <div style={{ display:'flex', gap:8, justifyContent:'center', flexWrap:'wrap' }}>
                  {['Jak funguje umělá inteligence?', 'Najdi mi fotky Prahy', 'Napiš mi báseň o moři', 'Najdi mi fotky hor'].map(hint => (
              ...
              {!isLoggedIn && (
                <button onClick={() => setShowAuth(true)} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'8px 16px', borderRadius:9, background:t.btn, border:`1px solid ${t.border}`, color:t.muted, fontSize:13 }}>
                  {Ic.user} <span>Přihlásit se pro uložení historie + vyhledávání fotek</span>
                </button>
              )}
            </div>
          )}
          ...
          {displayMsgs.map(msg => (
            <div key={msg.id} className="fi" style={{ display:'flex', gap:8, justifyContent:msg.role==='user'?'flex-end':'flex-start', alignItems:'flex-start' }}>
              ...
              <div style={{ maxWidth: msg.type === 'image_search' ? '92%' : '72%' }}>
                {msg.type === 'image_search' ? (
                  <div style={{ padding:'12px 14px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
                    <ImageSearchResults images={getImages(msg)} query={msg._query || msg.content} t={t}/>
          ...
          {loading && (
            <div className="fi" style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
              ...
              <div style={{ padding:'12px 16px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
                {imgSearchMode ? <span style={{ fontSize:13, color:t.muted }}>📷 Hledám fotografie…</span> : <div className="dot"><span/><span/><span/></div>}
              </div>
            </div>
          )}
        ...
        <div style={{ padding:'10px 14px 14px', background:t.iaBg, borderTop:`1px solid ${t.border}`, flexShrink:0 }}>
          {isLoggedIn && (
            <div style={{ display:'flex', gap:6, marginBottom:8, alignItems:'center' }}>
              <button onClick={() => setImgSearchMode(m => !m)}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:6, background:imgSearchMode?t.accent:t.btn, color:imgSearchMode?'#fff':t.muted, fontSize:12, border:`1px solid ${imgSearchMode?t.accent:t.border}`, fontWeight:imgSearchMode?600:400 }}>
                {Ic.imgSrch} {imgSearchMode ? '📷 Hledání fotek ZAP' : 'Hledat fotografie'}
              </button>
              <span style={{ fontSize:11, color:t.muted }}>
                {imgSearchMode ? 'Popište co hledáte' : 'nebo napište „najdi mi fotky…"'}
              </span>
            </div>
          )}
          ...
          <div style={{ display:'flex', alignItems:'flex-end', gap:6, padding:'9px 11px', background:t.inBg, border:`1.5px solid ${t.inBrd}`, borderRadius:14 }}>
            <textarea ref={taRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={onKey}
              placeholder={imgSearchMode ? '📷 Popište co hledáte… (např. "západ slunce nad mořem")' : 'Napište zprávu… (Enter = odeslat)'}
          ...
          <div style={{ fontSize:11, color:t.muted, textAlign:'center', marginTop:6 }}>
            Powered by Gemini AI + Unsplash{isLoggedIn ? ' · Historie uložena v Supabase' : ' · Přihlaste se pro uložení + vyhledávání fotek'}
          </div>
        </div>
      </main>
