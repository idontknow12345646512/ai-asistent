...
const ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNqZHZna2R2ZXp6ZmF6ZXh6ZnJmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQyMTYyODIsImV4cCI6MjA4OTc5MjI4Mn0.ZkZ9jImrSZDHAkSnAWPGgwXXkXEu4YnJtUbeyX99eOg'
...
function detectImageSearch(text) {
  ...
  return ['najdi obrázk', 'vyhledej obrázk', 'ukaž obrázk', 'zobraz obrázk',
    'hledej obrázk', 'obrázky ', 'fotky ', 'fotografie ', 'find image',
    'obrázek ', 'foto ', 'picture of', 'photo of'
  ].some(tr => t.includes(tr))
}
...
const Ic = {
  ...
  // Nová ikona pro generování obrázků — kouzelná hůlka s hvězdičkami
...
const T = {
  dark:  { bg:'#0f1117', side:'#13161f', hdr:'rgba(15,17,23,0.94)', txt:'#e8eaf0', muted:'#5a6178', border:'#1e2230', accent:'#6c8fff', active:'#1c2035', aiB:'#1a1d2a', inBg:'#13161f', iaBg:'#0f1117', inBrd:'#2a2f42', btn:'#1a1d2a', modal:'#13161f', pill:'#1a1d2a', ua:'#3d4460', scrl:'#2a2f42', card:'#1e2230' },
  light: { bg:'#f4f6fb', side:'#fff',    hdr:'rgba(244,246,251,0.94)', txt:'#1a1d2a', muted:'#7a849a', border:'#e2e6f0', accent:'#4c6ef5', active:'#eef1ff', aiB:'#fff',    inBg:'#fff',    iaBg:'#f4f6fb', inBrd:'#d8dde8', btn:'#edf0f7', modal:'#fff',    pill:'#edf0f7', ua:'#8898b0', scrl:'#c8cdd8', card:'#edf0f7' },
}

const mkLocalConv = () => ({ id: uid(), title: 'Nová konverzace', messages: [], createdAt: Date.now(), local: true })

// ── Unsplash Image Grid ────────────────────────────────────────────────────────
function ImageSearchResults({ images, query, t }) {
  const [errors, setErrors] = useState({})
  if (!images?.length) return <div style={{ fontSize:13, color:t.muted }}>🔍 Žádné fotografie nenalezeny pro „{query}"</div>
  return (
    <div>
      <div style={{ fontSize:12, color:t.muted, marginBottom:10, display:'flex', alignItems:'center', gap:6 }}>
        {Ic.imgSrch} Výsledky pro: <strong style={{ color:t.txt }}>„{query}"</strong>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:6 }}>
        {images.map((img, i) => !errors[i] && (
          <a key={img.id || i} href={img.source} target="_blank" rel="noopener noreferrer"
            style={{ display:'block', borderRadius:9, overflow:'hidden', border:`1px solid ${t.border}`, textDecoration:'none', background:t.card, transition:'transform .15s, opacity .15s' }}
            ...
            <div style={{ padding:'4px 7px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              <span style={{ color:t.muted }}>{Ic.extLink}</span>
            </div>
          </a>
        ))}
      </div>
      <div style={{ fontSize:10, color:t.muted, marginTop:8 }}>
        Fotografie z{' '}
        <a href="https://unsplash.com?utm_source=ai_asistent&utm_medium=referral" target="_blank" rel="noopener noreferrer" style={{ color:t.accent, textDecoration:'none', fontWeight:500 }}>Unsplash</a>
        {' '}· Klikni na foto pro zdroj
      </div>
    </div>
  )
}
...
// ── Generated Image ────────────────────────────────────────────────────────────
function GeneratedImage({ imageData, mimeType, prompt, t }) {
  ...
  return (
    <div>
      <div style={{ fontSize:12, color:t.muted, marginBottom:8, display:'flex', alignItems:'center', gap:6 }}>
        {Ic.magic} <span>Vygenerováno pomocí <strong style={{ color:t.accent }}>Imagen</strong></span>
      </div>
      <div style={{ position:'relative', display:'inline-block', maxWidth:'100%' }}>
        ...
        <button onClick={download}
          style={{ position:'absolute', top:8, right:8, display:'flex', alignItems:'center', gap:5, padding:'5px 10px', borderRadius:7, background:'rgba(0,0,0,0.6)', color:'#fff', fontSize:11, border:'none', cursor:'pointer', backdropFilter:'blur(4px)' }}>
          {Ic.download} Stáhnout
        </button>
      </div>
      <div style={{ fontSize:11, color:t.muted, marginTop:6 }}>„{prompt}"</div>
    </div>
  )
}
...
export default function Chat({ session }) {
  ...
  // mode: 'chat' | 'image_search' | 'generate_image'
  ...
  useEffect(() => {
    ...
    else { const c = mkLocalConv(); setConvs([c]); setActiveId(c.id); setMsgs([]) }
  ...
  async function loadDbConvs() {
    ...
    if (data?.length > 0) {
      ...
      setConvs(mapped); setActiveId(mapped[0].id)
  ...
  async function saveDbMsg(convId, role, content, type = 'text', meta = null) {
    const { data } = await supabase.from('messages').insert({
      conversation_id: convId, role, content, type,
  ...
  async function newConv() {
    ...
    if (isLoggedIn) {
      ...
      const c = mkLocalConv(); setConvs(p => [c, ...p]); setActiveId(c.id)
    }
  }
  ...
  async function delConv(id, e) {
    ...
    setConvs(prev => {
      ...
      const list = next.length > 0 ? next : [mkLocalConv()]
  ...
  const send = useCallback(async () => {
    ...
    // Určení režimu
    const wantsImgSearch   = isLoggedIn && (imgMode === 'image_search' || detectImageSearch(userText))
    const wantsImgGenerate = isLoggedIn && imgMode === 'generate_image'
    const apiMode = wantsImgGenerate ? 'generate_image' : wantsImgSearch ? 'image_search' : 'chat'
    ...
    const tmpUserMsg = { id: uid(), role: 'user', content: userText, type: 'text', created_at: new Date().toISOString(), _tmp: true }
    ...
    if (isLocal) {
      setConvs(p => p.map(c => {
        ...
        const title = c.title === 'Nová konverzace' && userText ? userText.slice(0, 38) + (userText.length > 38 ? '…' : '') : c.title
    ...
    try {
      const history = [...prevMsgs, tmpUserMsg].map(m => ({
        role: m.role,
        content: m.id === tmpUserMsg.id && apiContent.length > 0 ? apiContent : [{ type: 'text', text: m.content }]
      }))

      const token = isLoggedIn ? await getFreshToken() : ANON_KEY
      ...
      if (result.type === 'image_search') {
        aMsg = { id: uid(), role: 'assistant', type: 'image_search', content: `Nalezeno ${result.images?.length ?? 0} fotografií pro „${result.query}"`, _images: result.images, _query: result.query, image_url: JSON.stringify(result.images), created_at: new Date().toISOString() }
      } else if (result.type === 'generated_image') {
        aMsg = { id: uid(), role: 'assistant', type: 'generated_image', content: `🎨 Vygenerovaný obrázek: ${userText}`, _imageData: result.imageData, _mimeType: result.mimeType, _prompt: userText, image_url: JSON.stringify({ imageData: result.imageData, mimeType: result.mimeType, prompt: userText }), created_at: new Date().toISOString() }
      } else {
  ...
  // Placeholder texty podle režimu
  const placeholders = {
    chat: 'Napište zprávu… (Enter = odeslat)',
    image_search: '🔍 Popište co hledáte… (např. "západ slunce nad mořem")',
    generate_image: '🎨 Popište obrázek který chcete vygenerovat…',
  }

  // Loading texty
  const loadingTexts = {
    chat: null,
    image_search: '🔍 Hledám fotografie…',
    generate_image: '🎨 Generuji obrázek pomocí Imagen…',
  }
      ...
      <main style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <header style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', height:52, background:t.hdr, borderBottom:`1px solid ${t.border}`, backdropFilter:'blur(12px)', flexShrink:0 }}>
          ...
          <div style={{ display:'flex', gap:5, alignItems:'center' }}>
            ...
            {!isLoggedIn && <button onClick={() => setShowAuth(true)} style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 12px', borderRadius:7, background:t.accent, color:'#fff', fontSize:12, fontWeight:600 }}>{Ic.user}<span>Přihlásit se</span></button>}
          </div>
        </header>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 16px', display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ textAlign:'center', marginTop:'9vh' }}>
              ...
              {isLoggedIn && (
                <div style={{ display:'flex', gap:8, justifyContent:'center', flexWrap:'wrap' }}>
                  {['Jak funguje umělá inteligence?', 'Najdi mi fotky Prahy', 'Vygeneruj obrázek lesa', 'Napiš mi báseň o moři'].map(hint => (
                    <button key={hint} onClick={() => setInput(hint)}
                      style={{ padding:'6px 12px', borderRadius:20, background:t.btn, border:`1px solid ${t.border}`, color:t.muted, fontSize:12 }}>
                      {hint}
                    </button>
                  ))}
                </div>
              ...
              {!isLoggedIn && (
                <button onClick={() => setShowAuth(true)} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'8px 16px', borderRadius:9, background:t.btn, border:`1px solid ${t.border}`, color:t.muted, fontSize:13 }}>
                  {Ic.user} <span>Přihlásit se pro uložení + obrázky</span>
                </button>
              )}
            </div>
          )}
          ...
          {displayMsgs.map(msg => {
            ...
            return (
              <div key={msg.id} className="fi" style={{ display:'flex', gap:8, justifyContent:msg.role==='user'?'flex-end':'flex-start', alignItems:'flex-start' }}>
                ...
                <div style={{ maxWidth: ['image_search','generated_image'].includes(msg.type) ? '92%' : '72%' }}>
                  {msg.type === 'image_search' ? (
                    <div style={{ padding:'12px 14px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
                      <ImageSearchResults images={m._images} query={m._query || msg.content} t={t}/>
                      <div style={{ fontSize:10, color:t.muted, marginTop:8, textAlign:'right' }}>{fmtTime(msg.created_at)}</div>
                    </div>
                  ) : msg.type === 'generated_image' ? (
                    <div style={{ padding:'12px 14px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
                      <GeneratedImage imageData={m._imageData} mimeType={m._mimeType || 'image/png'} prompt={m._prompt || msg.content} t={t}/>
                      <div style={{ fontSize:10, color:t.muted, marginTop:8, textAlign:'right' }}>{fmtTime(msg.created_at)}</div>
                    </div>
                  ) : (
          ...
          {loading && (
            <div className="fi" style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
              ...
              <div style={{ padding:'12px 16px', background:t.aiB, borderRadius:'16px 16px 16px 4px', border:`1px solid ${t.border}` }}>
                {loadingTexts[imgMode]
                  ? <span style={{ fontSize:13, color:t.muted }}>{loadingTexts[imgMode]}</span>
          ...
          {err && <div style={{ padding:'9px 13px', background:'#ff444418', border:'1px solid #ff444440', borderRadius:9, fontSize:13, color:'#ff6b6b', display:'flex', gap:8 }}><span>⚠️</span><span>{err}</span></div>}
          <div ref={endRef}/>
        </div>
        ...
        <div style={{ padding:'10px 14px 14px', background:t.iaBg, borderTop:`1px solid ${t.border}`, flexShrink:0 }}>
          ...
          {/* Režimy — jen pro přihlášené */}
          {isLoggedIn && (
            <div style={{ display:'flex', gap:6, marginBottom:8, alignItems:'center' }}>
              {/* Generovat obrázek */}
              <button className="mode-btn" onClick={() => setImgMode(m => m === 'generate_image' ? 'chat' : 'generate_image')}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:6, fontSize:12, fontWeight: imgMode==='generate_image' ? 600 : 400, background: imgMode==='generate_image' ? '#a855f7' : t.btn, color: imgMode==='generate_image' ? '#fff' : t.muted, border: `1px solid ${imgMode==='generate_image' ? '#a855f7' : t.border}` }}>
                {Ic.magic} {imgMode === 'generate_image' ? '🎨 Imagen ZAP' : 'Generovat obrázek'}
              </button>
              {/* Hledat fotky */}
              <button className="mode-btn" onClick={() => setImgMode(m => m === 'image_search' ? 'chat' : 'image_search')}
                ...
                {Ic.imgSrch} {imgMode === 'image_search' ? '🔍 Hledání ZAP' : 'Hledat fotografie'}
              </button>
              {imgMode !== 'chat' && (
                <span style={{ fontSize:11, color:t.muted }}>
                  {imgMode === 'generate_image' ? 'Popište obrázek' : 'Popište co hledáte'}
                </span>
              )}
            </div>
          )}
          ...
          <div style={{ display:'flex', alignItems:'flex-end', gap:6, padding:'9px 11px', background:t.inBg, border:`1.5px solid ${imgMode==='generate_image'?'#a855f7':imgMode==='image_search'?t.accent:t.inBrd}`, borderRadius:14, transition:'border-color .2s' }}>
            ...
            <div style={{ display:'flex', gap:4, alignItems:'center' }}>
              ...
              <button onClick={send} disabled={!canSend}
                style={{ width:32, height:32, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', background: canSend ? (imgMode==='generate_image'?'#a855f7':t.accent) : t.btn, color:canSend?'#fff':t.muted, transition:'all .15s', flexShrink:0 }}>
                {Ic.send}
              </button>
            </div>
          </div>
          ...
          <div style={{ fontSize:11, color:t.muted, textAlign:'center', marginTop:6 }}>
            Powered by Gemini AI + Imagen + Unsplash{isLoggedIn ? ' · Historie uložena v Supabase' : ' · Přihlaste se pro plné funkce'}
          </div>
        </div>
      </main>
