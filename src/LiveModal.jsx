...
function pcm16ToFloat32(buf) {
  const int16 = new Int16Array(buf), float32 = new Float32Array(int16.length)
...
export default function LiveModal({ t, onClose, sysPmt, token }) {
  ...
  const playQueueRef = useRef([]), playingRef = useRef(false)
  ...
  const playAudio = async (b64) => {
    ...
    setAiSpeaking(false); playingRef.current = false
  }
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        proc.onaudioprocess = e => {
          ...
          const f32 = e.inputBuffer.getChannelData(0), pcm16 = new Int16Array(f32.length)
          for (let i = 0; i < f32.length; i++) pcm16[i] = Math.max(-32768, Math.min(32767, Math.round(f32[i]*32767)))
          const energy = f32.reduce((s,v)=>s+v*v,0)/f32.length
          setUserSpeaking(energy > 0.001)
          ws.send(JSON.stringify({ realtimeInput:{ mediaChunks:[{ mimeType:'audio/pcm;rate=16000', data:bufToB64(pcm16.buffer) }] } }))
        }
  ...
  const disconnect = () => {
    ...
    setStatus('idle'); setAiSpeaking(false); setUserSpeaking(false)
  }
  useEffect(() => () => disconnect(), []) // eslint-disable-line

  const modeOpts = [{id:'voice',icon:'🎤',label:'Hlas'},{id:'camera',icon:'📷',label:'Kamera'},{id:'screen',icon:'🖥️',label:'Obrazovka'}]

  return (
    <>
      <div onClick={status==='idle'||status==='error'?onClose:undefined} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',zIndex:98,backdropFilter:'blur(8px)'}}/>
      <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',zIndex:99,width:'min(520px,96vw)',background:t.modal,border:`1px solid ${t.border}`,borderRadius:20,fontFamily:"'DM Sans',sans-serif",overflow:'hidden'}}>
        <div style={{padding:'18px 20px 14px',borderBottom:`1px solid ${t.border}`,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <button onClick={status==='connected'?disconnect:onClose} style={{padding:'6px 14px',borderRadius:8,background:status==='connected'?'#ef4444':t.btn,color:status==='connected'?'#fff':t.muted,fontSize:13,fontWeight:500,border:'none',cursor:'pointer',fontFamily:'inherit'}}>
        ...
        <div style={{padding:'16px 20px'}}>
          {status!=='connected'&&(
            <div style={{marginBottom:16}}>
              ...
              <div style={{display:'flex',gap:8}}>
                {modeOpts.map(m=>(
                  <button key={m.id} onClick={()=>setMode(m.id)} style={{flex:1,padding:'12px 8px',borderRadius:10,border:`2px solid ${mode===m.id?t.accent:t.border}`,background:mode===m.id?t.accent+'22':t.btn,color:mode===m.id?t.accent:t.muted,fontSize:12,fontWeight:mode===m.id?600:400,cursor:'pointer',fontFamily:'inherit',transition:'all .15s',textAlign:'center'}}>
          ...
          {status==='connected'&&(
            <div style={{display:'flex',gap:12,marginBottom:14,justifyContent:'center'}}>
              {[{icon:'🎤',label:'Ty',active:userSpeaking,color:'#22c55e'},{icon:'🤖',label:'AI',active:aiSpeaking,color:t.accent}].map(item=>(
          ...
          {err&&<div style={{marginBottom:14,padding:'10px 13px',borderRadius:9,background:'#2a1a1a',border:'1px solid #6a2d2d',color:'#fca5a5',fontSize:13}}>⚠️ {err}</div>}
          {status!=='connected'&&(
            <button onClick={connect} disabled={status==='connecting'} style={{width:'100%',padding:'13px 0',borderRadius:12,background:t.accent,color:'#fff',fontSize:15,fontWeight:600,border:'none',cursor:status==='connecting'?'default':'pointer',fontFamily:'inherit',opacity:status==='connecting'?0.7:1,transition:'all .2s'}}>
              {status==='connecting'?'⏳ Připojuji…':'🎙️ Spustit Live'}
            </button>
          )}
          <div style={{marginTop:10,fontSize:11,color:t.muted,textAlign:'center'}}>Powered by Gemini 2.0 Flash Live · Limit ~10 min/den (free tier)</div>
        </div>
      </div>
    </>
  )
