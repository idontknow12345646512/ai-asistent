...
export const AI_MODELS=[
  {id:'default',       name:'Gemma 4 31B ✦',           short:'G4',   desc:'Výchozí — nejnovější Gemma 4'},
  {id:'gemini-flash',  name:'Gemini 3.1 Flash Lite ⚡', short:'G3.1', desc:'Nejrychlejší — lightweight'},
...
export const LIVE_MODELS=[
  {id:'gemini-3-flash-live',    name:'Gemini 3 Flash Live',           desc:'Nejnovější Live model'},
  {id:'gemini-25-native-audio', name:'Gemini 2.5 Flash Native Audio', desc:'Nativní audio dialog'},
  {id:'gemini-2-flash-live',    name:'Gemini 2.0 Flash Live',         desc:'Stabilní Live model'},
]
...
export const POLLEN_WINDOW=90

// Web Search typy
export const WEB_SEARCH_TYPES=[
  {id:'web',  label:'🌐 Web',    desc:'Celý internet'},
  {id:'video',label:'🎬 Videa',  desc:'YouTube a ostatní'},
  {id:'image',label:'🖼️ Obrázky',desc:'Obrázky z webu'},
]
...
export const fmtRelTime=ts=>{
  const diff=Date.now()-new Date(ts).getTime(),min=Math.floor(diff/60000)
  if(min<1)return'právě teď'
  if(min<60)return`před ${min} min`
  const h=Math.floor(min/60);if(h<24)return`před ${h} h`
...
// Datum/čas kontext pro Lumi
export const getNowCtx=()=>{
  const now=new Date()
  const days=['neděle','pondělí','úterý','středa','čtvrtek','pátek','sobota']
  return`\n\nDnes je ${days[now.getDay()]} ${now.toLocaleDateString('cs-CZ',{day:'numeric',month:'long',year:'numeric'})}, ${now.toLocaleTimeString('cs-CZ',{hour:'2-digit',minute:'2-digit'})}.`
}
...
// Pollen cache
export const getPollenCache=()=>{try{const d=JSON.parse(localStorage.getItem('lumi_pollen')||'{}');return{remaining:d.remaining??POLLEN_LIMIT,spent:d.spent||0,resetAt:d.resetAt||null,ts:d.ts||0}}catch{return{remaining:POLLEN_LIMIT,spent:0,resetAt:null,ts:0}}}
export const setPollenCache=d=>localStorage.setItem('lumi_pollen',JSON.stringify({...d,ts:Date.now()}))

// ── Auto-paměť: detekuje zda uložit info ze zprávy ───────────────────────────
export function detectMemoryInfo(text){
  const t=text.trim()
  // Explicitní zapamatování
  const explRe=/(zapamatuj si|zapamatuj|nezapomeň|remember that|note that|ulož si)\s+(.+)/i
  const em=t.match(explRe)
  if(em)return{save:true,content:em[2].trim().slice(0,200),category:'general'}
  // Jméno
  const nameRe=/(jmenuji se|mé jméno je|my name is)\s+([A-ZÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-záčďéěíňóřšťúůýž]{2,})/i
  const nm=t.match(nameRe)
  if(nm)return{save:true,content:`Uživatel se jmenuje ${nm[2]}`,category:'personal'}
  // Preference/práce — jen kratší přímé výroky
  if(t.length<120){
    if(/(mám rád\b|miluji\b|oblíbený\b|moje oblíbená)/i.test(t))return{save:true,content:t.slice(0,150),category:'prefs'}
    if(/(pracuji jako|jsem programátor|jsem student|studuji na)/i.test(t))return{save:true,content:t.slice(0,150),category:'work'}
  }
  return{save:false}
}
...
export function detectIntent(text){
  ...
  if(/(hledej na webu|vyhledej na webu|najdi na internetu|search the web)/i.test(t))return'web_search'
  // Generování — POUZE jasné příkazy s klíčovým slovem + obrázek/nakresli
  if(/(^vygeneruj\s+mi\s+obrázek|^vygeneruj\s+obrázek|^nakresli\s+mi|^nakresli\s+obrázek|vytvoř\s+obrázek\s+|generate\s+image|generate\s+a\s+picture|draw\s+me\s+a|create\s+image)/i.test(t))return'generate_image'
  // Foto
  if(/(najdi\s+fotku|najdi\s+fotografii|find\s+a\s+photo)/i.test(t))return'image_search'
  return'chat'
}
...
export function detectAutoMode(text,imgMode){
  ...
  if(['quiz','weather'].includes(intent))return'chat'
  return intent
}
...
export function renderMD(text,isDark){
  ...
  return text
    .replace(/```(\w*)\n?([\s\S]*?)```/g,(_,lang,code)=>{
      const lbl=lang?`<div style="display:flex;justify-content:space-between;padding:5px 12px;background:${isDark?'#161b22':'#eaeef2'};border-bottom:1px solid ${cbrd};font-size:11px;color:${isDark?'#8b949e':'#57606a'};font-family:monospace;border-radius:8px 8px 0 0"><span style="font-weight:600">${lang}</span></div>`:''
